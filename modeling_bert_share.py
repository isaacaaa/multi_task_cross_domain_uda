        ...
        if classifier=='classifier1':
            if labels is not None:
                if self.num_labels == 1:
                    ...
                    # if sim_labels!=None:
                        # relevant_loss = loss_fct(logits3.view(-1, 2), sim_labels.view(-1))
        if classifier=='classifier3':
            if labels is not None:
                if self.num_labels == 1:
                    #  We are doing regression
                    loss_fct = MSELoss()
                    relevant_loss = loss_fct(logits3.view(-1), labels.view(-1))
                else:
                    loss_fct = CrossEntropyLoss()
                
                    relevant_loss = loss_fct(logits3.view(-1, self.num_labels), labels.view(-1))
                    # if sim_labels!=None:
                        # relevant_loss = loss_fct(logits3.view(-1, 2), sim_labels.view(-1))
        if not return_dict:
            output = (logits1,) + outputs[2:]
            return ((loss,) + output) if loss is not None else output
        # if classifier=='classifier1' and sim_labels!=None:
